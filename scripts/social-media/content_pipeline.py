...
RSS_SOURCES = {
    "crypto": [
        ...
        {"name": "金色財經",       "url": "https://www.jinse.cn/rss",                  "lang": "zh"},
...
def fetch_bird_trending(channel: str) -> list[dict]:
    """用 bird 搜尋 Twitter/X 熱門推文，作為補充新聞來源"""
    query = TWITTER_QUERIES.get(channel, "")
    if not query:
        return []
    try:
        print(f"   🐦 bird 搜尋推特：{query[:50]}...")
        ...
        if articles:
            print(f"   ✅ Twitter：取得 {len(articles)} 則推文")
        return articles

    except FileNotFoundError:
        print("   ⚠️ bird 未安裝，跳過 Twitter 搜尋")
...
def score_article(article: dict, channel: str) -> int:
    """計算文章熱門分數：包含越多熱門關鍵字 = 分數越高"""
    keywords = HOT_KEYWORDS.get(channel, [])
    text = (article.get("title", "") + " " + article.get("description", "")).lower()
    score = sum(1 for kw in keywords if kw.lower() in text)
    # 描述品質加權：描述不足的扣 2 分，降低被選中機率
...
_CLAUDE_SYSTEM = {
    "crypto": "你是「幣圈大小事」IG 帳號的資深社群編輯，服務台灣 20-35 歲加密貨幣投資者。寫作風格：繁體中文、台灣口語、有深度、數字精確（如「高達 2.3 億美元」）、CTA 要有具體可回答的問題。寫作禁忌：嚴禁使用簡體字（必須用台灣正體繁體中文）、不可有翻譯腔、語句要口語通順像人說話，不要用「進行」「予以」「針對」等公文用語。",
    "finance": "你是「金融大小事」IG 帳號的資深社群編輯，服務台灣股市與金融投資人。寫作風格：繁體中文、台灣口語、有深度、數字精確、CTA 要有具體可回答的問題。寫作禁忌：嚴禁使用簡體字（必須用台灣正體繁體中文）、不可有翻譯腔、語句要口語通順像人說話，不要用「進行」「予以」「針對」等公文用語。",
    "startup": "你是「創業大小事」IG 帳號的資深社群編輯，服務台灣創業者與科技愛好者。寫作風格：繁體中文、台灣口語、有深度、數字精確、CTA 要有具體可回答的問題。寫作禁忌：嚴禁使用簡體字（必須用台灣正體繁體中文）、不可有翻譯腔、語句要口語通順像人說話，不要用「進行」「予以」「針對」等公文用語。",
}
...
def call_ai_claude(prompt: str, channel: str = "crypto") -> str | None:
...
def call_ai(prompt: str, timeout: int = 120, max_retries: int = 3) -> str | None:
    """透過 openclaw agent 呼叫 MiniMax AI（含自動重試）"""
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # ★ 跳過 plugin/log 前綴行（如 [plugins] ...），定位 JSON 起始
            json_start = raw.find('{')
            if json_start < 0:
                if result.stderr:
                    print(f"   ⚠️ openclaw stderr: {result.stderr[:200]}")
                if attempt < max_retries:
                    print(f"   ⚠️ MiniMax 回應無 JSON，{3}秒後重試 ({attempt}/{max_retries})...")
                    _time.sleep(3)
                continue
            ...
            if attempt < max_retries:
                print(f"   ⚠️ MiniMax 回應無 payload (status={status})，{3}秒後重試 ({attempt}/{max_retries})...")
                _time.sleep(3)
        except subprocess.TimeoutExpired:
            ...
            print(f"   ⚠️ MiniMax JSON 解析錯誤：{e}，前100字：{raw[:100]}")
            if attempt < max_retries:
                _time.sleep(3)
        except Exception as e:
            print(f"   ⚠️ AI 呼叫失敗：{e}")
...
def _claude_fallback(title: str, desc: str, ref_block: str,
                     ch_zh: str, source_name: str, channel: str) -> dict | None:
...
def ai_rewrite(article: dict, channel: str,
               related_articles: list[dict] | None = None) -> dict:
    """
    ...
    if related_articles:
        ...
        for i, ra in enumerate(related_articles[:4], 1):
            ...
            ra_title = ra.get("title", "")[:60]
            ra_desc = ra.get("description", "")[:200]
            ref_lines.append(f"【{ra_src}】{ra_title}\n{ra_desc}")
        ref_block = "\n\n同主題其他媒體報導（務必交叉引用不同觀點）：\n" + "\n\n".join(ref_lines)

    prompt = f"""你是台灣頂級{ch_zh}媒體 IG 社群編輯，專門寫 Carousel 10頁深度內容。
你的內容要像記者專題報導一樣紮實有深度，每一頁都要塞滿有價值的資訊，絕對不能有空洞感。
請把以下新聞改寫成繁體中文，台灣年輕人喜歡的口語風格，有衝擊力、接地氣、但有專業深度。
如果原文是英文，請翻譯成中文再改寫，務必補充足夠背景說明讓台灣讀者完全看得懂。
⚠️ 重要：每個欄位都必須寫滿要求字數下限！字數不夠的會被退回重寫。內容要紮實有料，有數據有觀點有分析。
⚠️ 如果有多來源報導，務必整合不同媒體的觀點和數據，讓分析更全面。

主要新聞（{source_name}）：
標題：{title}
摘要：{desc}{ref_block}

請直接輸出以下格式（每行一個欄位，冒號後面直接接內容，不要多餘說明）：

HOOK: 抓眼球標題（15字以內，引發好奇或震驚感，像新聞頭條一樣吸睛）
WHAT: 完整說明發生了什麼事（350-500字！要有完整來龍去脈：事件背景、觸發原因、具體經過、關鍵數據（金額/百分比/時間點）、當事人表態、各方反應，口語化但資訊密度要極高，讀者看完這頁就能完全掌握事件全貌）
EVIDENCE: 用一句話描述最能佐證這則新聞的畫面（30字以內，例如「比特幣價格暴跌走勢圖」「川普在白宮記者會發言」）
POINT1: 短標題（8字以內）|深度分析（200-300字！要有：1.具體數據佐證 2.背後原因分析 3.影響範圍評估 4.專家或業內觀點引用，像一篇迷你專欄文章）
POINT2: 短標題（8字以內）|深度分析（200-300字！從不同角度切入，補充第一點沒提到的面向，引用不同來源的數據或觀點）
POINT3: 短標題（8字以內）|深度分析（200-300字！聚焦對一般投資人/讀者的實際影響，給出具體情境和數字，讓讀者有切身感）
POINT4: 短標題（8字以內）|深度分析（200-300字！提出反方觀點或不同立場的論述，平衡報導，引述持不同意見者的看法和理由）
POINT5: 短標題（8字以內）|深度分析（200-300字！展望未來走向，分析接下來可能的劇情發展、潛在風險和機會，給出時間軸預測）
IMPACT: 為什麼這件事很重要（350-500字！全面深度分析：1.對整個產業/市場的長期影響 2.對台灣投資人的具體建議 3.歷史上類似事件的結果對比 4.後市展望與風險提醒 5.需要關注的後續指標或時間節點）
CONTEXT: 歷史脈絡（250-350字！列出 2-3 個過去的類似事件做對比，說明當時發生了什麼、結果如何、這次有何不同，讓讀者建立時間軸概念）
CHART_NOTE: 用一句話解釋近期走勢（例如「因 ETF 獲批推動，ETH 本週上漲 5%」，30字以內）
CTA: 互動問題（20字以內，引導留言討論，讓粉絲有話說）"""
    ...
    def _parse_ai_output(output: str, data: dict) -> dict:
        ...
        - HOOK：內容          (全形冒號)
        - **HOOK:** 內容      (Markdown 粗體)
        - **HOOK**：內容      (Markdown + 全形冒號)
        """
        for line in output.splitlines():
            # 統一處理：全形冒號→半形、移除 markdown bold markers
    ...
    # ── 拆成兩次 AI 呼叫（避免輸出截斷）──
    # 第一輪：HOOK + WHAT + EVIDENCE + POINT1-3
    prompt_part1 = f"""你是台灣頂級{ch_zh}媒體 IG 社群編輯，專門寫 Carousel 10頁深度內容。
你的內容要像記者專題報導一樣紮實有深度，每一頁都要塞滿有價值的資訊，絕對不能有空洞感。
請把以下新聞改寫成繁體中文，台灣年輕人喜歡的口語風格，有衝擊力、接地氣、但有專業深度。
如果原文是英文，請翻譯成中文再改寫，務必補充足夠背景說明讓台灣讀者完全看得懂。
⚠️ 重要：每個欄位都必須寫滿要求字數下限！字數不夠的會被退回重寫。內容要紮實有料，有數據有觀點有分析。
⚠️ 如果有多來源報導，務必整合不同媒體的觀點和數據，讓分析更全面。
⚠️ 嚴禁出現任何簡體字！全部使用台灣正體繁體中文！
⚠️ 語句務必通順自然！用台灣人日常說話的方式寫，避免翻譯腔和機器感。
  - 不要用「進行」「予以」「針對」「根據」等公文用語，改用白話
...
HOOK: 抓眼球標題（15字以內，引發好奇或震驚感，像新聞頭條一樣吸睛）
WHAT: 完整說明發生了什麼事（350-500字！要有完整來龍去脈：事件背景、觸發原因、具體經過、關鍵數據（金額/百分比/時間點）、當事人表態、各方反應，口語化但資訊密度要極高，讀者看完這頁就能完全掌握事件全貌）
EVIDENCE: 用一句話描述最能佐證這則新聞的畫面（30字以內）
POINT1: 短標題（8字以內）|深度分析（200-300字！要有：1.具體數據佐證 2.背後原因分析 3.影響範圍評估 4.專家或業內觀點引用，像一篇迷你專欄文章）|📊 關鍵數據（一句話，必須包含具體數字）
POINT2: 短標題（8字以內）|深度分析（200-300字！從不同角度切入，補充第一點沒提到的面向，引用不同來源的數據或觀點）|📊 關鍵數據（一句話）
POINT3: 短標題（8字以內）|深度分析（200-300字！聚焦對一般投資人/讀者的實際影響，給出具體情境和數字，讓讀者有切身感）|📊 關鍵數據（一句話）"""
    ...
    # 第二輪：POINT4-5 + IMPACT + CONTEXT + CTA
    prompt_part2 = f"""你是台灣頂級{ch_zh}媒體 IG 社群編輯，續寫深度分析。
以下新聞你已經分析了前三個重點，現在繼續寫後半部分。
⚠️ 每個欄位都必須寫滿要求字數下限！
⚠️ 嚴禁出現任何簡體字！全部使用台灣正體繁體中文！
⚠️ 語句務必通順自然！用台灣人日常說話的方式寫，避免翻譯腔。
  - 不要用「進行」「予以」「針對」等公文用語，改用白話
...
POINT4: 短標題（8字以內）|深度分析（200-300字！提出反方觀點或不同立場的論述，平衡報導，引述持不同意見者的看法和理由）|📊 關鍵數據（一句話）
POINT5: 短標題（8字以內）|深度分析（200-300字！展望未來走向，分析接下來可能的劇情發展、潛在風險和機會，給出時間軸預測）|📊 關鍵數據（一句話）
IMPACT: 為什麼這件事很重要（350-500字！全面深度分析：1.對整個產業/市場的長期影響 2.對台灣投資人的具體建議 3.歷史上類似事件的結果對比 4.後市展望與風險提醒 5.需要關注的後續指標或時間節點）
CONTEXT: 歷史脈絡（250-350字！列出 2-3 個過去的類似事件做對比，說明當時發生了什麼、結果如何、這次有何不同，讓讀者建立時間軸概念）
CHART_NOTE: 用一句話解釋近期走勢（例如「因 ETF 獲批推動，ETH 本週上漲 5%」，30字以內）
CTA: 互動問題（20字以內，引導留言討論，讓粉絲有話說）"""
    ...
    def _is_quality_ok(d: dict) -> bool:
        """檢查 AI 回傳品質是否合格"""
        hook = d.get("hook", "")
        if not _has_zh(hook):
            print(f"   ❌ 品質檢查：HOOK 無中文 → '{hook[:30]}'")
            return False
        points = d.get("points", [])
        if len(points) < 3:
            print(f"   ❌ 品質檢查：points 不足 3 個（只有 {len(points)}）")
            return False
        ...
        if len(real_points) < 3:
            print(f"   ❌ 品質檢查：有效 points 不足 3 個（{len(real_points)}/{len(points)}）")
            for i, p in enumerate(points):
                has_zh = _has_zh(p)
                ok_len = len(p) > 20
                print(f"      PT{i+1}: zh={has_zh} len={len(p)} → '{p[:40]}'")
            return False
        return True
    ...
    print("   ⚠️ MiniMax 回傳品質不足，嘗試 Claude API fallback...")
    claude_data = _claude_fallback(title, desc, ref_block, ch_zh, source_name, channel)
    if claude_data and _is_quality_ok(claude_data):
    ...
    if data.get("hook") and _has_zh(data["hook"]):
        ...
        data["hook"] = title[:15] if _has_zh(title) else f"幣圈快訊"
    ...
    points = data.get("points", [])
    # 保留有實質中文內容的 points
    good_points = [p for p in points if _has_zh(p) and len(p) > 20]
    if desc:
...
def run_pipeline(channel: str, dry_run: bool = False) -> bool:
    ...
    print(f"\n   嘗試 Twitter 補充...")
    twitter_arts = fetch_bird_trending(channel)
    ...
    print(f"\n   📚 共收集 {len(all_articles)} 篇文章，開始評分篩選...")
    article = pick_best_article(all_articles, channel)
    if not article:
    ...
    # ── Step 2：AI 改寫 ──────────────────────────────────────
    print("\n[2/4] AI 改寫文案...")
    ai_data = ai_rewrite(article, channel, related_articles=related)
...
{hashtags}"""
    ...
    def _check_quality_gate() -> bool:
        """檢查整體文案品質，不合格就不發文"""
        issues = []
        if not _has_chinese(hook, 0.05):  # HOOK 只要有中文就好（英文人名多）
            issues.append("HOOK 不含中文")
        if not _has_chinese(what, 0.1):
            issues.append("WHAT 不含中文")
        # 檢查 points 品質
        points = ai_data.get("points", [])
        fallback_count = sum(1 for p in points if "持續關注後續" in p or len(p) < 15)
        if fallback_count >= 3:
            issues.append(f"{fallback_count}/5 個重點是佔位文字")
        if issues:
            print(f"\n   🚫 品質閘門未通過：")
            for iss in issues:
                print(f"      ❌ {iss}")
    ...
    if dry_run:
        ...
        person_name = ai_data.get("person", "")
        ...
        if candidates:
            print(f"\n   🖼️ 封面候選（{len(candidates)} 張）：")
            for c in candidates:
                print(f"      {c}")
            print("   💡 用 understand_image 看圖後選最好的一張")
...
def publish_last_dryrun(channel: str) -> bool:
    ...
    ig_caption = caption_path.read_text().strip()
