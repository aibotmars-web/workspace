...
def fetch_commodity_chart(symbol: str = "GC=F", label: str = "黃金",
                          days: int = 7,
                          size: tuple = (960, 500)) -> tuple[Image.Image | None, dict]:
    """用 Yahoo Finance API 取得商品走勢，繪製與 crypto 相同風格的極簡漸層面積圖。
    支援黃金(GC=F)、白銀(SI=F)、原油(CL=F) 等期貨商品。"""
    try:
        import json as _json
        from urllib.request import urlopen as _urlopen, Request as _Req
...
def _pick_diverse_cover_prompts(mood: str, count: int = 3,
    ...
    for i in range(count):
        ...
        # 確保每個候選用不同風格
        available_styles = [s for s in _STYLE_MODIFIERS if s not in used_styles]
        if not available_styles:
            available_styles = list(_STYLE_MODIFIERS)
        style = random.choice(available_styles)
        used_styles.append(style)

        prompt = base.rstrip().rstrip(",") + f", {style}"
...
def slide_evidence(bg: Image.Image, theme: dict, evidence_text: str,
    ...
    if chart_img:
        ...
        src_label = "Yahoo Finance" if detected_commodity else "CoinGecko"
...
def slide_single_point(bg: Image.Image, theme: dict, point_num: int,
    ...
    # desc_block: desc_h
    # gap between title and desc: padding(50) is already in content_total
    actual_content_h = title_block_h + desc_h + data_box_h + padding
    if data_lines:
        actual_content_h += 24  # gap before data box
...
def generate_carousel(channel: str, ai_data: dict, article_url: str,
    ...
    while len(points) < 5:
        points.append(f"延伸分析|此新聞值得持續關注後續發展")
